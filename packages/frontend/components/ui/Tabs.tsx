...
function TabItem({ tab, selected }: { tab: Tab; selected: boolean }) {
    return (
        <button
            className={clsx(
                'mr-8 whitespace-no-wrap py-4 px-1 border-b-2 border-transparent font-medium text-sm leading-5 focus:outline-none',
                selected
...
function PillsTabItem({
    ...
    return (
        <button
            className={clsx(
                'mr-4 py-2 px-3 font-medium text-sm leading-5 rounded-md focus:outline-none',
                !secondary && {
                    ...
                    <nav className={clsx('flex', !pills && '-mb-px')}>
                        {tabs.map(tab =>
                            pills ? (
                                ...
                                <TabItem tab={tab} selected={value === tab.value} />
                            )
                        )}
                    </nav>
                </div>
